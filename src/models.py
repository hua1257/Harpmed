"""
模块职责：
1. 汇总项目中复用的基础模型组件与多种基线模型实现。
2. 包含图卷积层封装、分子图编码器，以及 SafeDrug、DMNC、GAMENet、Leap、Retain 等模型。
3. 为各训练脚本提供可直接实例化的网络结构定义。
"""
...
class MaskLinear(nn.Module):
    ...
    def forward(self, input, mask):
        if mask.shape != self.weight.shape:
            if mask.t().shape == self.weight.shape:
                mask = mask.t()
            else:
                raise RuntimeError(
                    "MaskLinear shape mismatch: weight shape {} vs mask shape {}.".format(
                        tuple(self.weight.shape), tuple(mask.shape)
                    )
                )
...
class MolecularGraphNeuralNetwork_mod(nn.Module):
    ...
    def pad(self, matrices, pad_value):
        ...
        zeros = torch.zeros((M, N), dtype=torch.float32, device=self.device)
...
class DMNC(nn.Module):
    def __init__(self, vocab_size, emb_dim=64, device=torch.device('cpu:0')):
        ...
        if DNC is None:
            raise ImportError("DMNC requires the optional 'dnc' package.")
        K = len(vocab_size)
        self.K = K
        self.vocab_size = vocab_size
        self.device = device
        dnc_gpu_id = device.index if device.type == 'cuda' else -1
    ...
    def move_state_to_device(self, state):
        if state is None:
            return None
        if torch.is_tensor(state):
            return state.to(self.device)
        return type(state)(self.move_state_to_device(item) for item in state)

    def forward(self, input, i1_state=None, i2_state=None, h_n=None, max_len=20):
        # input (3, code)
        i1_state = self.move_state_to_device(i1_state)
        i2_state = self.move_state_to_device(i2_state)
        h_n = self.move_state_to_device(h_n)
